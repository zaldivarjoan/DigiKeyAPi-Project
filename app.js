...
export const handleKeywordSearch = async (keyword, useCache = false) => {
  try {
    ...
    if (error.handleKeywordSearch == null) {
...
const displayItemDetails = (itemDetails) => {
  ...
  console.log(
    `Manufacturer Product ID :  ${itemDetails.Product.Manufacturer.Id}`
  );
  console.log(
    `Manufacturer Product Number :  ${itemDetails.Product.ManufacturerProductNumber}`
  );
